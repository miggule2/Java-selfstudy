...
### 1. 정렬에서의 구분
* __추가적인 공간 필요 여부__
  * ```out-of-place 정렬``` : 모든 데이터를 __자료 구조의 복사본으로 옮긴 후__ 순서대로 배열하여 정렬하는 방법,
  * ```in-place 정렬``` : __자료 구조를 그대로 두고__ 그 안에서 요소의 위치를 바꾸어 정렬하는 방법

* __정렬 후 중복된 요소의 위치 변화__
  * ```stable sort(안정 정렬)``` : 요소 내에 중복된 요소가 있을 때, __정렬 후에 중복된 요소간의 순서가 유지됨__
  * ```unstable sort(불안정 정렬)``` : __정렬 후에 중복된 요소 간의 순서가 바뀜.__

* __시간 복잡도__
  * 모든 정렬은 __최악, 평균, 최선의 경우의 복잡도__ 를 모두 봐야한다.
  * 최악의 경우는 보통 반대로 정렬된 경우이고, 최선의 경우는 이미 정렬이 완료된 경우를 뜻함.

### 2. 선택 정렬
* 1~n 번째까지 넘어가며 __각 위치에 확정되지 않은 숫자 중 가장 작은 수를 채워가는 방법.__
* n-1 ~ 2 까지 합해주면 복잡도가 나오고
* __시간복잡도__ : 최악,평균,최선의 경우 모두 ```O(n^2)```.
```java
...
### 3. 삽입 정렬
* __요소를 하나씩 꺼내어, 그 앞의 다른 요소들과 비교하는 정렬__
* __In-place 정렬__
* __시간복잡도__ : 최악, 평균의 경우엔 앞 요소(1,2,..,n-1) 개와 비교해야 하기 때문에 ```O(N^2)```이지만, 최선의 경우(이미 정렬된 경우)는 비교를 각각 1번씩만 하면 되기 때문에 ```O(n)```
* 이러한 특성 때문에, 삽입 정렬은 __이미 정렬된 데이터베이스에 요소를 추가하는 경우__ 에 활용된다.
```java
...
### 4. 쉘 정렬
* 쉘 정렬은 __간격 + 삽입 정렬__.
* 배열을 일정한 간격으로 나누어 각 간격의 요소들끼리 삽입 정렬을 하고, 그 간격을 줄여가는 과정을 반복하는 정렬.
* __불안정 정렬__
* __In-place 정렬__
...
### 7. 기수 정렬
* __정의__ : __자릿수 등의 기준을 만들어 기준에 따라 숫자를 분류하는 정렬 방법__
* 예를 들어, 4자리 숫자의 경우 일의자리 -> 십의자리 -> 백의자리 -> 천의자리의 숫자에 따라 분류하여 정렬.
* __시간복잡도__ : 이론적으로는 ```O(n)```의 시간 복잡도를 가지지만, __각 기준에 따라 정렬할 때마다 숫자를 복사하여 실제로는 매우 오래걸림
